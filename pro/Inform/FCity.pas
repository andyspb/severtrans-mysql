...
begin
  ...
  begin
    ...
    if edistance.text = '' then
      ...
      str := str + ',' + sql.MakeStr(trim(edistance.text));
    //----
    ...
    if egdstrah.text = '' then
      ...
      str := str + ',' + sql.MakeStr(trim(egdstrah.text));
    //-----------------
    ...
    if sql.insertstring('City', 'Ident,Name,Tariff200,Tariff500,Tariff1000,' +
      'Tariff2000,TariffMore2000,Sending,Distance,Check,gdstrah,DaysDel', str)
        <> 0 then
      Addrecord := 0
    else
      AddRecord := l;
...
begin
  ...
  if q.FieldByName('Check').AsInteger = 1 then
  ...
  begin
    ...
    if edistance.text = '' then
      ...
      str := str + ',distance=' + sql.MakeStr(trim(edistance.text));
    //----------
    if CheckBox1.Checked then
      str := str + ',`Check`=' + IntToStr(1)
    else
      str := str + ',`Check`=' + IntToStr(0);
    //-----------
    if egdstrah.text = '' then
      str := str + ',gdstrah=NULL'
    else
      str := str + ',gdstrah=' + sql.MakeStr(trim(egdstrah.text));
    //----------
    if LabelInteger1.text = '' then
      str := str + ',DaysDel=0'
    else
      str := str + ',DaysDel=' + LabelInteger1.text;
    //-----------------
    if sql.updatestring('City', str, 'Ident=' + IntToStr(Id)) = 0 then
      EditRecord := Id
    else
      EditRecord := 0;
    //----
...
begin
  if CheckBox1.Focused and CheckBox1.Checked then
    CheckBox1.Checked := False
  else if CheckBox1.Focused then
    CheckBox1.Checked := true;
end;
