...
uses
  ...
  Dialogs, tsqlcls, Lebtn, Sqlctrls, Lbsqlcmb, StdCtrls, ExtCtrls, Buttons,
    BMPBtn,
  ...
  begin
   ...
   // begin
    ...
    if ReasId <> 0 then
      ...
      s := S + ', OnReason_Ident=' + 'NULL';

    if ClTypeId <> 0 then
      s := s + ', ClientType_Ident=' + IntToStr(ClTypeId)
    else
      s := S + ', ClientType_Ident=' + 'NULL';
    ...
    if sql.ExecOneSql('Update Clients set ' + s + ' where Ident=' + IntToStr(Id)
      + '; ') <> 0 then
    begin
      Addrecord := 0;
      exit;
...
begin
  ...
  PersTypeId := sql.SelectInteger('Clients', 'PersonType_Ident', 'Ident=' +
    intToStr(l));
  countryId := sql.SelectInteger('Clients', 'Country_Ident', 'Ident=' +
    intToStr(l));
  ...
  UrAddr := sql.Selectstring('Address', 'AdrName', 'Clients_Ident=' + intToStr(l)
    + ' and AddressType_Ident=1');
  FacAddr := sql.Selectstring('Address', 'AdrName', 'Clients_Ident=' +
    intToStr(l) + ' and AddressType_Ident=2');
  ...
  CalCount := sql.Selectstring('Clients', 'CalculatCount', 'Ident=' +
    intToStr(l));
  ...
  ReasId := sql.SelectInteger('Clients', 'OnReason_Ident', 'Ident=' +
    intToStr(l));
  ClTypeId := sql.SelectInteger('Clients', 'ClientType_Ident', 'Ident=' +
    intToStr(l));
  ...
  st := FormatDateTime('yyyy-mm-dd', StrToDate(sql.Selectstring('Clients',
    'Start', 'Ident=' + intToStr(l))));
  if ClTypeId = 1 then
  begin
    q := sql.Select('Contract', 'Number', 'Clients_Ident=' + intToStr(l) +
      ' and ContractType_Ident=2', '');
    if not q.eof then
      ContrAv := sql.Selectstring('Contract', 'Number', 'Clients_Ident=' +
        intToStr(l) + ' and ContractType_Ident=2') + ', ' +
        FormatDateTime('mm.dd.yyyy', StrToDate(sql.Selectstring('Contract',
          'Start', 'Clients_Ident=' +
        intToStr(l) + ' and ContractType_Ident=2')))
    else
      ContrAv := '';
 ...
 //  cond:=' and Phone='+sql.MakeStr(phone)
  ...
  qc := sql.select('Contact', 'Name', 'Clients_Ident=' + intToStr(l) +
    cond, '');
  if not qc.eof then
    ContactName := qc.fieldByName('Name').asString
  else
    ContactName := '';
  qc.free;
  ...
  LabeledEdit18.Text := sql.Selectstring('Clients', 'KPP', 'Ident=' +
    intToStr(l));
  ...
  LabeledEdit13.text := FormatDateTime('dd.mm.yyyy',
    StrToDate(sql.Selectstring('Clients', 'Start', 'Ident=' + intToStr(l))));
  LabeledEdit14.text := FormatDateTime('dd.mm.yyyy', now);
  ...
  if sql.SelectInteger('Clients', 'NameGood_Ident', 'Ident=' + intToStr(l)) <> 0
    then
    cbGryz.SetActive(sql.SelectInteger('Clients', 'NameGood_Ident', 'Ident=' +
      intToStr(l)));
  cbForwarder.SQLComboBox.where := 'Clients_Ident=' + IntToStr(ID);
  //cbForwarder.SQLComboBox.recalc;
  if sql.Selectstring('Clients', 'Forwarder_Ident', 'Ident=' + intToStr(l)) <> ''
    then
    cbForwarder.SQLComboBox.setactive(sql.SelectInteger('Clients',
      'Forwarder_Ident', 'Ident=' + IntToStr(Id)));
  ...
  if sql.Selectstring('Clients', 'KreditTEK', 'Ident=' + intToStr(l)) <> '' then
    LabelEdit1.Text := sql.Selectstring('Clients', 'KreditTEK', 'Ident=' +
      intToStr(l));
  ...
  begin
    ...
    begin
      ...
      if sql.ExecOneSql('Update Contact set ' + s + ' where Clients_Ident=' +
        IntToStr(id)) <> 0 then
      begin
        editRecord := 0;
        exit;
    ...
    begin
      ...
      if phone <> '' then
        ...
        s := s + ',' + 'NULL';
      s := s + ',' + IntToStr(id);
      if sql.ExecOneSql('Insert into Contact values(' + s + ');') <> 0 then
      begin
        editRecord := 0;
        exit;
      end;
    ...
    qad.free;
    qad := sql.Select('Address', '', 'Clients_Ident=' + IntToStr(id) +
      ' and AddressType_Ident=2', '');
    if not qad.eof then
    begin
      if FacAddr <> '' then
      ...
      if sql.ExecOneSql('Update Address set ' + s + ' where Clients_Ident=' +
        IntToStr(id) +
    ...
    begin
      ...
      begin
        ...
        if sql.insertstring('Address',
          'AdrName,Clients_Ident,AddressType_Ident', s) <> 0 then
        begin
          editRecord := 0;
          exit;
        end;
...
begin
  ...
  begin
    Application.MessageBox
      ('Укароченное название клиента не должно превышать 35 символов!',
        'Ошибка', 0);
...
begin
  ...
  begin
    Application.MessageBox
      ('Укороченное название клиента не должно превышать 25 символов!',
        'Ошибка', 0);
...
begin
  if Length(LabeledEdit6.text) > 30 then
  begin
    Application.MessageBox
...
begin
  ...
  begin
    Application.MessageBox
      ('Описание представителя клиента не должно превышать 50 символов!',
        'Ошибка', 0);
  ...
  begin
    ...
    begin
      ...
      if sql.InsertString('Forwarder', 'Ident,Name,Clients_Ident',
        IntToStr(l) + ',' + sql.MakeStr(val) + ',' + IntToStr(Id)) <> 0 then
      begin
        exit
      end
      else
...
begin
  ...
  begin
    ...
    eFullName.Hint :=
      'Введите полное название клиента (не должено превышать 70 символов)';
    LabeledEdit2.EditLabel.Caption := 'Короткое название клиента ';
    LabeledEdit2.Hint :=
      'Введите короткое название клиента (не должено превышать 25 символов)';
