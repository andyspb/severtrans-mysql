...
begin
  ...
  begin
    l := sql.FindNextInteger('Ident', EntrySec.accounttek_table {'`AccountTEK`'},
      '', MaxLongint);
    str := IntToStr(L);
    ...
    if sql.insertstring(EntrySec.accounttek_table {'`AccountTEK`'}, fields, str)
      = 0 then
    begin
      AddRecord := l;
      // success
      // update other table
      ins_thread := TInsertThread.Create(True, EntrySec.accounttek_table_other,
        fields, str);
      ins_thread.Resume();
    end
    else
...
begin
  ...
  FormCreate;
  I := Iden;
  q := sql.Select(EntrySec.accounttek_table {'`AccountTEK`'}, '*', 'Ident=' +
    IntToStr(Iden), '');
...
begin
  ...
  q := sql.select(EntrySec.accounttekview_view {'`AccountTEKView`'},
    'Number,`Year`', '`Year`=' + IntToStr(Year), '');
  if q.eof then
...
begin
  try
    ...
    if ReportMakerWP.DoMakeReport(systemdir + 'Account\AccountTEK.rtf',
      systemdir + 'Account\AccountTEK.ini', systemdir + 'Account\out.rtf') <> 0
        then
    begin
      ReportMakerWP.Free;
      //goto T;
    ...
    if InputQuery('Диалог!', 'Какое количество копий счета распечатать?', i1,
      ...
      w1 := i1;
    if i1 = 0 then
      w1 := 1; {если не задано, то печаем одну копию}
    // w1:=2;
    mach := '';
