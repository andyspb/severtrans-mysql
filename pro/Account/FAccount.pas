...
begin
  ...
  begin
    l := sql.FindNextInteger('Ident', EntrySec.account_table {'`Account`'}, '',
      MaxLongint);
    str := IntToStr(L);
    ...
    begin
      ...
      AddRecord := l;
      // update other table
      thread := TInsertThread.Create(True, EntrySec.account_table_other, fields,
        str);
      thread.Resume();
...
begin
  ...
  FormCreate;
  I := Iden;
  q := sql.Select(EntrySec.account_table {'`Account`'}, '*', 'Ident=' +
    IntToStr(Iden), '');
...
begin
  ...
  q := sql.select(EntrySec.accountview_view {'`AccountView`'}, 'Number,`Year`',
    '`Year`=' + IntToStr(Year), '');
  if q.eof then
  ...
  begin
    ...
    Num := IntToStr(N1 + 1) + '/' + FormatDateTime('yy',
      StrToDate(LabelEditDate1.text));
  end;
...
begin
  try
    ...
    if (ReportMakerWP.DoMakeReport(systemdir + 'Account\Account.rtf',
      systemdir + 'Account\Account.ini', systemdir + 'Account\out.rtf') <> 0)
        then
    begin
      ReportMakerWP.Free;
      Logger.LogError(EntrySec.version +
        '[FAccount] [Print] Failed ReportMakerWP.DoMakeReport');
      //goto T;
      exit
    end;
    ReportMakerWP.Free;
    ...
    i3 := 5;
    i4 := 1;
    if InputQuery('Диалог!', 'Какое количество копий счета распечатать?', i1,
      i2, i3, i4) then
      w1 := i1;
    if (i1 = 0) then
      w1 := 1;
    ...
    begin
      Logger.LogError(EntrySec.version +
        '[FAccount] [Print] Exception happened: ' + E.Message);
      if (WordApplication1.Documents <> nil) then
      begin
        WordApplication1.Documents.Close(EmptyParam, EmptyParam, EmptyParam);
      end;
