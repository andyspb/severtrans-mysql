...
begin
  ...
  NumberChange := '';
  IdInv := Iden;
  q := sql.Select(EntrySec.invoice_table {'Invoice'}, '', 'Ident=' +
    IntToStr(IdInv), '');
  ...
  begin
    ...
    if (sql.UpdateString(EntrySec.invoice_table {'Invoice'}, s, 'Ident=' +
    ...
    begin
      // success
      // update other tables
      upd_thread := TUpdateThread.Create(True, EntrySec.invoice_table_other, s,
        'Ident=' + IntToStr(IdInv));
      upd_thread.Resume();
    end;
    if (NumberChange <> '') then
      Number := NumberChange;
    fields := 'NumberCountPattern=' + sql.MakeStr(Number) + ',' +
      'Invoice_Ident=' + IntToStr(IdInv);
    if (sql.UpdateString(EntrySec.send_table {'Send'}, fields, 'Ident in (' +
      StrIdSend + ')') <> 0) then
    begin
      sql.Rollback;
      EditRecord := 0;
...
begin
  Logger.LogInfo(EntrySec.version +
    '[FInvoice] (TFormInvoice.btPrintClick()) ');
  ...
  begin
    Application.MessageBox('Введите дату формирования счет фактуры!', 'Ошибка!',
      0);
  ...
  //------------------------------------------------------
  if (NumberChange <> '') then
    if (sql.SelectString(EntrySec.invoice_table {'Invoice'}, 'Number', 'Number='
...
begin
  ...
  try
    ...
    s1 := SendStr.DataDMstrY(StrToDate(Dat));
    ReportMakerWP.AddParam('2=' + s1);
    ...
    s1 := sql.SelectString('Bank', 'Name', 'Ident=' +
      IntToStr(query.FieldByName('Bank_Ident').asinteger));
    ReportMakerWP.AddParam('8=' + s1);
    s1 := sql.SelectString('Bank', 'KorCount', 'Ident=' +
      IntToStr(query.FieldByName('Bank_Ident').asinteger));
    ReportMakerWP.AddParam('9=' + s1);
    s1 := sql.SelectString('Bank', 'BIK', 'Ident=' +
      IntToStr(query.FieldByName('Bank_Ident').asinteger));
    ReportMakerWP.AddParam('10=' + s1);
  ...
  try
    ...
    s1 := sql.SelectString('Address', 'AdrName', 'Clients_Ident=' +
      IntToStr(cbClient.SQLComboBox.GetData) +
      ' and Addresstype_Ident=' + intToStr(1));
    ReportMakerWP.AddParam('17=' + s1);
    ...
    s1 := sql.SelectString('Bank', 'Name', 'Ident=' +
      IntToStr(query.fieldByName('Bank_Ident').asInteger));
    ReportMakerWP.AddParam('21=' + s1);
    s1 := sql.SelectString('Bank', 'KorCount', 'Ident=' +
      IntToStr(query.fieldByName('Bank_Ident').asInteger));
    ReportMakerWP.AddParam('22=' + s1);
    s1 := sql.SelectString('Bank', 'BIK', 'Ident=' +
      IntToStr(query.fieldByName('Bank_Ident').asInteger));
    ReportMakerWP.AddParam('23=' + s1);
    s1 := sql.SelectString('City', 'Name', 'Ident=' +
      IntToStr(query.fieldByName('City_Ident').asInteger));
    //---------------------------
    if (s1 <> '') then
    begin
      if (pos(' до ', s1) <> 0) or (pos(' склад', s1) <> 0) then
      begin
        delete(s1, pos(' ', s1), Length(s1) - pos(' ', s1) + 1);
      end;
    end;
    //---------------------------
    ReportMakerWP.AddParam('24=' + s1);
    ...
    Logger.LogError(EntrySec.version +
      '[TFormInvoice.Print] 2 sql.Select(Contract....');
    if (sql.SelectString('Contract', 'Number', 'Clients_Ident=' +
      IntToStr(Ident) +
      ' and `Start`<=' + sql.MakeStr(FormatDateTime('yyyy-mm-dd',
      StrToDate(Dat))) +
  ...
  s1 := SendStr.MoneyToString(StrTo00(FloatToStr(SumNDS)));
  ReportMakerWP.AddParam('35=' + s1);
  s1 := SendStr.MoneyToString(StrTo00(FloatToStr(NDS)));
  ReportMakerWP.AddParam('36=' + s1);
  ...
  try
    s1 := SendStr.MoneyToString(StrTo00(FloatToStr(Fee)));
    ReportMakerWP.AddParam('37=' + s1);
    s1 := 'Ident in (' + StrIdSend + ')';
    ReportMakerWP.AddParam('38=' + s1);
    invoice_ini := iff(EntrySec.bAllData, 'Invoice_all.ini', 'Invoice.ini');
    Logger.LogError(EntrySec.version + '[TFormInvoice.Print] invoice_ini=' +
      invoice_ini);

    //-----------------------------------------------------------------------------------------------
    // open close word - remove if it is not working
    Logger.LogError(EntrySec.version +
      '[TFormInvoice.Print] 1 TWordApplication.Create');
    ...
    begin
      Logger.LogError(EntrySec.version +
        '[FInvoice.Print] failed ReportMakerWP.DoMakeReport <> 0');
      // report failed
      ReportMakerWP.Free;
      Logger.LogError(EntrySec.version +
        '[FInvoice.Print] (invoice) ReportMakerWP.DoMakeReport() FAIL');
  ...
  try
    ...
    begin
      Logger.LogError(EntrySec.version +
        '[FInvoice.print] Failed to create Word App.');
    ...
    Logger.LogInfo(EntrySec.version +
      '[FInvoice.print] 2 TWordApplication.visible=true');
    ...
    //----------------------------
    if (Code = 2) then
      {при повторной печати даем возможность задать кол копий от 0 до 2}
    begin
      i1 := 0;
      i2 := 0;
      i3 := 2;
    ...
    user_name := trim(WordApplication1.UserName);
    w2 := sql.SelectString('Printer', 'NameA4', 'ComputerName=' +
      sql.MakeStr(user_name));
    Logger.LogInfo(EntrySec.version +
    ...
    begin
      Logger.LogError(EntrySec.version +
        '[FInvoice] (invoice) Missed printers info');
    ...
    //-----------------------------
    Logger.LogInfo(EntrySec.version +
      '[FInvoice.print] WordApplication1.ActivePrinter');
    WordApplication1.ActivePrinter := w2;
    Logger.LogInfo(EntrySec.version +
      '[FInvoice.print] WordApplication1.ActivePrinter.PrintOut');
    ...
    begin
      Logger.LogError(EntrySec.version +
        '[FInvoice.Print] T: WordApplication1.Documents.Close');
      ...
      begin
        try
          ...
          begin
            Logger.LogError(EntrySec.version +
              '[FInvoice.Print] T: Exceptions happened on WordApplication1.Documents.Close(): ' +
              E.Message);
          end;
        end;
      end;
      ...
      Logger.LogInfo(EntrySec.version +
        '[FInvoice.print] after T: WordApplication1.WindowState=2');
      WordApplication1.WindowState := 2;
      Logger.LogInfo(EntrySec.version +
        '[FInvoice.print] after T: WordApplication1.Free');
    ...
    s1 := 'Ident in (' + StrIdSend + ')';
    ReportMakerWP.AddParam('38=' + s1);
    certificate_ini := iff(EntrySec.bAllData, 'Certificate_all.ini',
      'Certificate.ini');
    //-----------------------------------------
    ...
    begin
      ReportMakerWP.Free;
      Logger.LogError(EntrySec.version +
        '[FInvoice.print] ReportMakerWP.DoMakeReport()<>0 failed.');
    ...
    begin
      Logger.LogError(EntrySec.version +
        '[FInvoice.print] (Application is null');
      application.MessageBox('Oшибка связи с приложением', 'Error', 0);
    end;

    Logger.LogError(EntrySec.version +
      '[FInvoice.print] TWordApplication.Create');
    WordApplication1 := TWordApplication.Create(Application);
    Logger.LogError(EntrySec.version +
      '[FInvoice.print] TWordApplication.Visible=true');
    WordApplication1.Visible := true;
    ...
    begin
      ...
      i3 := 2;
      i4 := 1;
      if (InputQuery('Диалог!', 'Какое количество копий акта распечатать?', i1,
        i2, i3, i4)) then
        w1 := i1;
    end;
    if (w1 = 0) then {не печатаем, если пользователь задаст "0" количество копий}
    ...
    begin
      Logger.LogError(EntrySec.version +
        '[FInvoice.print] Missed printers info');
      application.MessageBox('Информация о принтерах не внесена в базу для данной машины' +
        ' или в параметрах WinWord не верно указано имя машины!', 'Ошибка!', 0);
    ...
    Logger.LogError(EntrySec.version +
      '[FInvoice.print] w4:=WordApplication1.UserName');
    w4 := WordApplication1.UserName;
    if (w3 <> w4) then
    begin
      w2 := '\\' + w3 + '\' + w2;
    ...
    begin
      Logger.LogError(EntrySec.version +
        '[FInvoice] [Print] WordApplication1.ActiveDocument NULL');
      application.MessageBox('Выходной документ MS Word не готов', 'Ошибка!',
        0);
      Logger.LogError(EntrySec.version + '[FInvoice.print] goto t1');
      goto T1;
    end;
    Logger.LogError(EntrySec.version +
      '[FInvoice.print] WordApplication1.ActiveDocument.PrintOut');
    WordApplication1.ActiveDocument.PrintOut(
      EmptyParam, EmptyParam, EmptyParam,
      EmptyParam, EmptyParam, EmptyParam,
      EmptyParam, w1, EmptyParam,
    ...
    begin
      Logger.LogError(EntrySec.version +
        '[FInvoice.Print] WordApplication1.Documents.Close');
      if (WordApplication1.Documents <> nil) then
      begin
        try
          WordApplication1.Documents.Close(EmptyParam, EmptyParam,
          ...
          begin
            Logger.LogError(EntrySec.version +
              '[FInvoice.Print] T1 Exceptions happened on WordApplication1.Documents.Close(): ' +
              E.Message);
          end;
        end;
      end;
      Logger.LogError(EntrySec.version +
        '[FInvoice.Print] WordApplication1.WindowState.2');
      WordApplication1.WindowState := 2;
      Logger.LogError(EntrySec.version +
        '[FInvoice.Print] WordApplication1.Free');
      WordApplication1.Free;
    end;
    ...
    begin
      ...
      begin
        ...
        begin
          Logger.LogError(EntrySec.version +
            '[FInvoice.Print] E WordApplication1.Documents.Close');
          try
            WordApplication1.Documents.Close(EmptyParam, EmptyParam,
              EmptyParam);
          except
            ...
            begin
              Logger.LogError(EntrySec.version +
                '[FInvoice.Print] Exceptions happened on WordApplication1.Documents.Close(): ' +
                E.Message);
            end;
          end;
        end;
        Logger.LogError(EntrySec.version +
          '[FInvoice.Print] E WordApplication1.WindowState=2');
        WordApplication1.WindowState := 2;
        Logger.LogError(EntrySec.version +
          '[FInvoice.Print] E WordApplication1.Free');
        WordApplication1.Free;
      end;
      application.MessageBox('Проверьте все настройки для печати!', 'Error!',
        0);
...
begin
  ...
  l := sql.FindNextInteger('Ident', EntrySec.invoice_table {'Invoice'}, '',
    MaxLongint);
  str := IntToStr(l);
  ...
  if (sql.InsertString(EntrySec.invoice_table {'Invoice'}, insert_fields, str)
    <> 0) then
  begin
    sql.Rollback;
    exit;
  ...
  begin
    // success
    // update other tables
    ins_thread := TInsertThread.Create(True, EntrySec.invoice_table_other,
      insert_fields, str);
    ins_thread.Resume();
  end;
  ...
  begin
    upd_thread := TUpdateThread.Create(True, EntrySec.send_table_other,
      update_fields, 'Ident in (' + StrIdSend + ')');
    upd_thread.Resume();
  end;
end;
...
begin
  ...
  q := sql.select(EntrySec.invoiceview_view {'InvoiceView'}, 'Number,`Year`',
    '`Year`=' + IntToStr(Year), '');
  if (q.eof) then
