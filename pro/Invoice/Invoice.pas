    ...
    begin
      ...
      begin
        ...
        if q.fieldByName('AddServiceExp').AsInteger = 1 then
          f1 := f1 + q.fieldByName('ExpCount').AsFloat *
            q.fieldByName('ExpTarif').AsFloat;
        if q.fieldByName('AddServiceProp').AsInteger = 1 then
          f1 := f1 + q.fieldByName('PropCount').AsFloat *
            q.fieldByName('PropTarif').AsFloat;
        perc := 0;
      ...
      else if Typ = 2 then {авто}
      begin
        str1 := '';
        str1 := q.fieldByName('Weight').Asstring;
        if str1 <> '' then
          str1 := ', ' + str1 + ' кг.'; {вес отправки для печати в с/ф}
        ...
        if q.fieldByName('AddServiceProp').AsInteger = 1 then
          f1 := f1 + q.fieldByName('PropCount').AsFloat *
            q.fieldByName('PropTarif').AsFloat;
        if StrToDate(q.FieldByName('Start').AsString) > StrToDate('31.05.2012')
          then
      ...
      begin
        ...
        if (q.fieldByName('CityName').AsString <> '') then
          ...
          str := str + ',NULL' + q.fieldByName('Namber').AsString;
...
begin
  test := false;
  PT := sql.selectinteger('Clients', 'PersonType_Ident', 'Ident=' +
    IntToStr(Id));
  if PT = 1 then
  begin
    q1 := sql.select('Clientstek', 'Acronym', 'Ident=' + IntToStr(Id), '');
    if q1.eof then
      q := sql.Select(EntrySec.send_table {'Send'},
        'DateSupp,Ident,CountInvoice', 'Client_Ident=' + IntToStr(Id) +
        ' and NumberCountPattern is NULL and( (ContractType_Ident=2 and ' +
...
begin
  ...
  begin
    ...
    if sql.UpdateString(EntrySec.invoice_table {'Invoice'}, s, 'Ident=' +
      IntToStr(qIn.fieldByName('Ident').asInteger)) <> 0 then
    begin
      sql.Rollback;
...
{$I-}
  Writeln(c,
    'Номера отправок, в которых заказчик не совпадает с заказчиком в с/ф:#13#1010');
  CloseFile(c);
  //---------------------------
...
{$I-}
  Writeln(j,
    'Номера и данные счет-фактур, у которых старые значения не сошлись с новыми.#13#1010');
  Writeln(j,
    'В первой строке перечисляются старые данные, во второй новые.#13#1010');
  Writeln(j, ' ');
  ...
  qIn := sql.Select(EntrySec.invoiceview_view {'InvoiceView'}, '',
    'Data > ''2003-12-31''', 'Num');
  Num2 := 0;
...
{$I-}
    ...
    q := sql.Select(EntrySec.sends_view {'Sends'},
      'DateSupp,Ident,CountInvoice,Client_Ident,Namber,ClientAcr', 'Invoice_Ident='
      +
      qIn.fieldByName('Ident').asString, 'DateSupp');
    if not q.Eof then
    begin
...
{$I-}
   ...
   { If FloatToStr(SumNDS)<>FloatToStr(SumControl) then
    ...
    if (StrTo00(FloatToStr(SumGd)) <> qIn.FieldByName('SumGd').asString) or
      ...
      (StrTo00(FloatToStr(NDSStAg)) <> qIn.FieldByName('NDSStAg').asString) then
      Test := true;
    s := '';
    s1 := '';
    if Test then
...
begin
  ...
  begin
    ...
    if (q.fieldByName('InsuranceValue').Asstring <> '0.00') and
      (q.fieldByName('InsuranceValue').Asstring <> '') then
      f1 := f1 + q.fieldByName('InsuranceValue').Asfloat;
