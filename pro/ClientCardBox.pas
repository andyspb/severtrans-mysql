...
begin
  Sql.StartTransaction;
  Ident := SQLGrid1.Query.FieldByName('Ident').Asinteger;
  if sql.execOneSql('Delete from Clients where Ident=' + IntToStr(Ident)) <> 0
    then
  begin
...
begin
  ...
  if sql.InsertSelect('Clients',
    'Ident,Name,Acronym,FullName,Telephone,Fax,Email,INN,' +
  ...
  begin
    if sql.InsertSelect('Contact', 'Name,Phone,Clients_Ident', 'Contact',
      'Name,Phone,' +
    ...
    begin
      ...
      begin
        if sql.InsertSelect('Address',
          'AdrName,AddressType_Ident,Clients_Ident',
          'Address', 'AdrName,AddressType_Ident,' +
          IntToStr(Id), 'Clients_Ident=' + IntToStr(Ident)
        ...
        begin
          ...
          if sql.InsertSelect('Contract',
            'Ident,Number,`Start`,Finish,ContractType_Ident,Clients_Ident',
            'Contract', IntToStr(Id1) +
              ',Number,`Start`,Finish,ContractType_Ident,' +
            IntToStr(Id), 'Clients_Ident=' + IntToStr(Ident)
