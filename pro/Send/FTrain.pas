...
begin
  ...
  else if cbxList.combobox.ItemIndex = 1 then
  begin
    Query1.Close;
    Query1.DatabaseName := sql.DataBaseName;
    Query1.SQL.Clear;
...
begin
  ...
  begin
    ...
    if sql.UpdateString('Train', 'Arch=' + IntToStr(1), 'Ident=' + Inttostr(l))
      = 0 then
    begin
      Query1.Close;
      Query1.DatabaseName := sql.DataBaseName;
...
begin
  ...
  begin
    l := Query1.FieldValues['Ident'];
    if sql.UpdateString('Train', 'Arch=' + IntToStr(0), 'Ident=' + Inttostr(l))
      = 0 then
    begin
      Query1.Close;
      Query1.DatabaseName := sql.DataBaseName;
