  ...
  begin
    ...
    begin
      q := sql.select(EntrySec.send_table {'Send'},
        'SumCount,Client_Ident,PayType_Ident', 'Client_Ident=' +
        IntToStr(Id) + ' and PayType_Ident=' + IntToStr(l) +
        ' and `Start` >= ' + sql.MakeStr(FormatDateTime('yyyy-mm-dd', DateStart))
          +
        ' and `Start`<= ' + sql.MakeStr(FormatDateTime('yyyy-mm-dd', DateFin)),
...
begin
  ...
  begin
    ...
    begin
      ...
      q.Free;
      q := sql.select(EntrySec.order_table {'`Order`'}, 'SumNDS,Client_Ident',
        'Client_Ident=' + IntToStr(Id), '');
      while not q.eof do {считаем сумму по всем приходникам +}
      begin
    ...
    begin
      ...
      q.Free;
      q := sql.select(EntrySec.paysheet_table {'PaySheet'}, 'Sum,Client_Ident',
        'Client_Ident=' + IntToStr(Id), '');
      while not q.eof do {считаем сумму по всем платежкам +}
      begin
...
begin
  ...
  if id <> 0 then
  begin
    q := sql.select(EntrySec.send_table {'Send'}, 'SumCount,Client_Ident',
      'Client_Ident=' + IntToStr(Id), '');
    ...
    begin
      ...
      if pos('-', q.FieldByName('Sum').asstring) = 0 then
        Sum1 := Sum1 + q.FieldByName('Sum').asfloat
      else if (FloatToStr(Sum1) = s3) then
        Sum1 := 0
      else
        Sum1 := Sum1 + q.FieldByName('Sum').asfloat;
      q.Next;
...
begin
  ...
  begin
    ...
    if (S1 = s2) then
      ...
      CreditUpdate := FloatToStr(Sum1 - Sum);
...
begin
  ...
  begin
    ...
    begin
      if pos('-', q.FieldByName('Sum').asstring) = 0 then
        {учитываем только положительные}
        Sum1 := Sum1 + q.FieldByName('Sum').asfloat;
      q.Next;
    end;
    q.Free;
    ...
    if (S1 = S2) then
      ...
      CreditDate2004 := FloatToStr(Sum1 - Sum);
...
begin
  ...
  begin
    ...
    begin
      ...
      begin
        if m1 < 12 then
          dat := encodeDate(y1, m1 + 1, StrToInt(s))
        else
          dat := encodeDate(y1 + 1, 1, StrToInt(s))
      end;
      if Dat <= Dat1 then
        Dat1 := Dat;
    end;
